...
function updateHealth(player) {
    ...
    const baseHealth = player.getAttribute('minecraft:generic.max_health').getBaseValue();
    const maxHealth = Math.max(2, baseHealth - (deaths * 2));
...
EntityEvents.death(event => {
    ...
    player.server.scheduleInTicks(10, () => {
        updateHealth(player);
        player.tell(
            Text.translate('message.soul_fading', [
                Text.of(deaths + 1).color('red')
            ]).color('red')
        );
    });
});
...
ItemEvents.foodEaten(event => {
    ...
    if (['minecraft:golden_apple', 'minecraft:enchanted_golden_apple', 'kubejs:bad_apple'].includes(event.item.id)) {
        ...
        if (deaths > 0) {
            ...
            updateHealth(player);
            player.tell(
                Text.translate('message.soul_healing', [
                    Text.of(deaths - 1).color('green')
                ]).color('green')
            );
        }
    }
